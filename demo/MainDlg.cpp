...
struct ANIMATORDATA
{
    ANIMATORDATA(CMainDlg *_pMainWnd,BOOL bFadein)
    {
        pMainDlg = _pMainWnd;
        pToolbar = _pMainWnd->FindChildByName(L"webkit_toolbar");
        SASSERT(pToolbar);
        if(bFadein) InitFadein();
        else InitFadeout();
    }
...
#define TIMER_TOOLBAR_SHOW  1000
#define TIMER_TOOLBAR_HIDE  2000

void OnToolbarAnimatorState(int percent,LPARAM lp)
{
    ANIMATORDATA *pAniData = (ANIMATORDATA*)lp;
    if(percent == SAnimator::ABORT)
    {
        percent = SAnimator::PER_END;
    }
    
    BYTE byAlpha = (BYTE)(pAniData->byStart + (pAniData->byEnd-pAniData->byStart)*percent/100);
    
    STRACE(L"onanistate,alpha=%d",byAlpha);

    SWindow *pChild = pAniData->pToolbar->GetWindow(GSW_FIRSTCHILD);
    while(pChild)
    {
        pChild->SetVisible(byAlpha >0,TRUE);
        pChild->SetAttribute(L"alpha",SStringW().Format(L"%u",byAlpha));//修改窗口的alpha属性
        pChild=pChild->GetWindow(GSW_NEXTSIBLING);
    }
    if(percent == SAnimator::PER_END) 
    {
        if(pAniData->byStart == 0)
            pAniData->pMainDlg->CSimpleWnd::KillTimer(TIMER_TOOLBAR_SHOW);
        else
            pAniData->pMainDlg->CSimpleWnd::KillTimer(TIMER_TOOLBAR_HIDE);
        delete pAniData;
    }
}


void CMainDlg::OnWebkitToolbar_Hover( EventArgs *pEvt )
{
    m_toolbarAnimator.Stop();
    ANIMATORDATA  *pAniData = new ANIMATORDATA(this,TRUE);
    m_toolbarAnimator.SetAnimatorHandler(OnToolbarAnimatorState,(LPARAM)pAniData);
    m_toolbarAnimator.Start(200);
    CSimpleWnd::SetTimer(TIMER_TOOLBAR_SHOW,0,NULL);
}

void CMainDlg::OnWebkitToolbar_Leave( EventArgs *pEvt )
{
    m_toolbarAnimator.Stop();
    ANIMATORDATA  *pAniData = new ANIMATORDATA(this,FALSE);
    m_toolbarAnimator.SetAnimatorHandler(OnToolbarAnimatorState,(LPARAM)pAniData);
    m_toolbarAnimator.Start(200);
    CSimpleWnd::SetTimer(TIMER_TOOLBAR_HIDE,0,NULL);
}

void CMainDlg::OnTimer( UINT_PTR idEvent )
{
    if(idEvent == TIMER_TOOLBAR_SHOW || idEvent == TIMER_TOOLBAR_HIDE)
    {
        m_toolbarAnimator.Update();
    }else
    {
        //调用SHostWnd::OnTimer(idEvent);或者SetMsgHandled(FALSE);将定时器消息交给基类处理
        SHostWnd::OnTimer(idEvent);//   or      SetMsgHandled(FALSE);
    }
}